  ...
  requestPrice(shareUrl, function (data) {
    if (data) {
      ...
      if (tradeConsumerProtection) {
        tradeConsumerProtection = setTradeConsumerProtection(
          data,
        ...
        if (vertical && vertical.hasOwnProperty("tmallhkDirectSale")) {
          ...
          value.tradeConsumerProtection = setTradeConsumerProtection(
            data,
            value.tradeConsumerProtection
          );
        } else {
...
function priceSummary(data) {
  ...
  list.forEach((item, index) => {
    ...
    summary += `\n${item.Name}${getSpace(4)}${item.Price}${getSpace(4)}${
      item.Date
    }${getSpace(4)}${item.Difference}`;
    let summaryItem = `${item.Name}${getSpace(4)}${item.Price}${getSpace(4)}${
      item.Date
    }${getSpace(4)}${item.Difference}`;
